...
**协议统一**：
- ESP32/Node/STM32 全部使用 simple 协议（`F,<ms>` / `B,<ms>` / `L,<ms>` / `R,<ms>` / `S`）
- 安全阈值配置化：`danger: 8cm`, `caution: 15cm`, `safe: 30cm`
- 固件能力声明API：`/api/hardware/status` 返回能力开关

**OTA远程升级（已验证可用）**：
```
开发者修改代码 → pio run编译 → 更新版本号 → ESP32自动拉取更新 → 重启运行新版本
```
- ESP32每5分钟检查Node后端 `/api/ota/check`
- 检测到新版本自动下载固件并更新
- **已实际验证**：v2.4.0 → v2.4.1 OTA成功

**设备动态注册**：
- ESP32启动后向Node后端注册：`POST /api/esp32/register`
- 注册信息：MAC地址、IP地址、固件版本、运行时间
- 每60秒心跳，Node后端自动维护在线设备列表
- **无需手动配置ESP32 IP地址**
...
| OTA远程升级 | ✅ 完成 | /ota 手动上传 + Node后端自动拉取 |
| 设备动态注册 | ✅ 完成 | 启动注册 + 60秒心跳 |
| 协议统一 | ✅ 完成 | simple协议: F/B/L/R,<ms> + S |
| 启动流程 | ✅ 完成 | Phase 0-3 自检→网络→服务→就绪 |
| 硬件连线测试 | ✅ 完成 | GPIO4/5 ↔ PA9/PA10，通信正常 |
...
### 视频流低延迟优化（2026-01-30 更新）

**优化措施**：
| 优化项 | 说明 |
|--------|------|
| `WiFi.setSleep(false)` | 关闭 WiFi 省电模式 |
| `esp_wifi_set_ps(WIFI_PS_NONE)` | IDF 层彻底关闭省电 |
| `WiFi.setTxPower(19.5dBm)` | 最大发射功率，减少丢包 |
| `CAMERA_GRAB_LATEST` | 永远取最新帧，避免旧帧堆积 |
| `fb_count = 3` | GRAB_LATEST 下有效帧数为 2 |
| `availableForWrite()` 丢帧 | TCP 缓冲不足时丢帧保实时 |
| `JPEG_QUALITY = 35` | 更高压缩率，减少数据量 |
| `20fps 上限` | 避免资源过度消耗 |
| `setNoDelay(true)` | 禁用 Nagle 算法 |
| 非阻塞状态机 | 巡逻模式不使用 delay() |
...
| API | 方法 | 功能 |
|-----|------|------|
| `/api/esp32/register` | POST | ESP32设备注册/心跳 |
| `/api/esp32/devices` | GET | 获取在线设备列表 |
| `/api/esp32/info` | GET | 获取ESP32当前信息 |
| `/api/ota/check` | GET | 检查OTA更新（ESP32调用） |
| `/api/ota/firmware` | GET | 下载固件文件 |
| `/api/hardware/status` | GET | 硬件状态+能力开关+安全阈值 |
...
3. **用户体验**
   - 通电后自动连接已存储的家庭WiFi ✅ 已完成
   - 自动检查OTA更新 ✅ 已完成
   - 硬件自检（Phase 0）→ LED/蜂鸣器反馈 ✅ 已完成
...
| 模块 | 型号 | 用途 | 状态 |
|------|------|------|------|
| 小车底盘 | STM32 + 红外 + 舵机 + 超声波 + 双马达 | 运动控制 | ✅ 已有 |
| 上位机 | ESP32-S3-N16R8 | WiFi + AI + 视觉 | ✅ 已有 |
| 摄像头 | OV3660 (300万像素) | 视觉识别 | ✅ 已有 |
| 麦克风 | INMP441 | 语音输入 | ✅ 已有 |
| 音频输出 | 放大器 + 扬声器 | 语音播放 | ✅ 已有 |
| 电源 | 5V移动电源 Type-C | ESP32供电 | ✅ 已有 |
| 连接线 | 杜邦线 | 各模块连接 | ✅ 充足 |

**连线方案详见**: `docs/hardware-wiring.md`
...
**ESP32-S3 与 STM32 连接方式（2026-01-26 已验证）**：
```
ESP32-S3          STM32 蓝色板
GPIO4 (TX)  -->  A10 (PA10/RX)
GPIO5 (RX)  <--  A9  (PA9/TX)
GND         ---  GND
```
⚠️ **注意**：GPIO43/44 被 USB-UART 调试占用，不能用于串口通信！

**ESP32-S3 硬件配置（已验证）**：
| 项目 | 配置 |
|------|------|
| 开发板 | ESP32-S3 WROOM + 摄像头模块 |
| Flash | 16MB |
| PSRAM | 8MB |
| STM32 串口 TX | GPIO4 |
| STM32 串口 RX | GPIO5 |
| 波特率 | 115200 |
| WiFi AP | SSID: Simo-Robot, 密码: simo1234 |
| 固件目录 | `esp32/src/main.cpp` |

**STM32 硬件配置（已验证）**：
| 项目 | 配置 |
|------|------|
| 主控 | STM32F103C8T6 蓝色最小系统板 |
| 扩展板 | ZY10A 绿色扩展板 |
| ESP32 串口 TX | PA9 (A9) |
| ESP32 串口 RX | PA10 (A10) |
| 电机 PWM | TIM4 (PB6/PB7/PB8/PB9) |
| 蜂鸣器 | PB0（设置为浮空输入避免长鸣） |
| 红外避障 | PA11(左), PA12(右) |
| 红外循迹 | PB12(左), PB13(右) |
| 超声波 | PB15(TRIG), PB14(ECHO) |
| 烧录工具 | FlyMcu, COM5, 256000bps |
| 固件目录 | `stm32/simo_minimal/main.c` |
| 编译项目 | 案例10（机器人蓝牙控制） |
